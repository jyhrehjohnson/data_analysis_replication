...
head(d1,3) #print the first 3 lines of data for visualization
```
...
head(d2,3) #print the first 3 lines of data for visualization
```
...
d3 <- read.csv(species_f, header = TRUE, stringsAsFactors = FALSE)
```
```{r d3 Output, collapse=FALSE}
head(d3,3) #print the first 3 lines of data for visualization
```
...
head(e1,3) #Print the first 3 lines of data for visualization

e1.scaled <- eigen(cov(bs_data_scaled))
head(e1.scaled,3) #Print the first 3 lines of data for visualization
...
head(bs_data_pc,3) #Print the first 3 lines of data for visualization

bs_pca <- prcomp(bs_data[, -1], #Use the bs_data removing the first column, leaving only numeric values in the dataset. 
             center = TRUE,
             scale. = TRUE)
head(bs_pca$x,3) #Print the first 3 lines of data for visualization
...
oj_data_scaled <- scale(oj_data[, -1], #Use the oj_data removing the first column, leaving only numeric values in the data set. 
                     center = TRUE, 
                     scale = TRUE)
head(oj_data_scaled,3) #Print the first 3 lines of data for visualization

e2 <- eigen(cov(oj_data[, -1])) #Use the oj_data removing the first column, leaving only numeric values in the data set. 
head(e2,3) #Print the first 3 lines of data for visualization

e2.scaled <- eigen(cov(oj_data_scaled))
head(e2.scaled,3) #Print the first 3 lines of data for visualization
...
head(oj_data_pc,3) #Print the first 3 lines of data for visualization

oj_pca <- prcomp(oj_data[, -1], #Use the oj_data removing the first column, leaving only numeric values in the data set. 
             center = TRUE,
             scale. = TRUE)
head(oj_pca$x,3) #Print the first 3 lines of data for visualization
...
mkt_plot <- ggplot(d3, aes(x = MKT)) +
       geom_density(aes(group = subfamily, fill = subfamily), alpha = 0.5) + 
       xlim(c(min(d3$MKT), c(max(d3$MKT)))) +
